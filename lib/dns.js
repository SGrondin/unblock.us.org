(function() {
  var CLASSES, NAMEERROR, SERVERFAILURE, TYPES, bitconcat, getAnswer, getClass, getType, make2Bytes, makeDNS, parse2Bytes, parse3Bytes, parseDNS, prependLength, redirected_types, redirection, settings;
  ...
  getAnswer = function(parsed, isTCP) {
    ...
    domain = settings.hijacked[parsed.QUESTION.NAME.slice(-2).join(".")] || settings.hijacked[parsed.QUESTION.NAME.slice(-3).join(".")] || null;
    if (domain != null) {
