(function() {
  ...
  hijackedDomain = function(name) {
    ...
    if ((name.slice(-1)[0] != null) && name.slice(-1)[0] === "unblock") {
      name.pop();
      ret.hostTunneling = true;
    }
    ret.domain = settings.hijacked[name.slice(-2).join(".")] || settings.hijacked[name.slice(-3).join(".")] || null;
    return ret;
  };
  ...
  getAnswer = function(parsed, isTCP) {
    ...
    analyzed = hijackedDomain(parsed.QUESTION.NAME.slice(0));
    if (analyzed.domain != null) {
